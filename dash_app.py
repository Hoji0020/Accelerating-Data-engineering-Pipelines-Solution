...
def update_map(date_value, show_zeros):
    ...
    print(f"Data for {date_value} before filtering:", ddf.head())
    ddf = ddf if show_zeros else ddf[ddf["Inches"] != 0]
    print(f"Data for {date_value} after filtering:", ddf.head())
    ddf = ddf.to_pandas()
...
def update_series(click_data):
    ...
    ddf = ddf.to_pandas()  # Convert only for Plotly
